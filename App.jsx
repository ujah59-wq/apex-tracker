...
async function storageGet(key) { try { if(window.storage) return await window.storage.get(key); const v=localStorage.getItem(key); return v?{value:v}:null; } catch{return null;} }
async function storageSet(key,value) { try { if(window.storage) await window.storage.set(key,JSON.stringify(value)); else localStorage.setItem(key,JSON.stringify(value)); } catch{} }
      ...
      <nav style={{position:"sticky",top:0,zIndex:200,background:"rgba(255,255,255,0.6)",backdropFilter:"blur(30px) saturate(200%)",WebkitBackdropFilter:"blur(30px) saturate(200%)",borderBottom:"1px solid rgba(255,255,255,0.8)",boxShadow:"0 2px 20px rgba(120,80,220,0.07)"}}>
        <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",padding:"8px 12px 0"}}>
          <div style={{display:"flex",alignItems:"center",gap:6}}>
            <div style={{width:26,height:26,borderRadius:8,background:"linear-gradient(135deg,#7c3aed,#2563eb)",display:"flex",alignItems:"center",justifyContent:"center",boxShadow:"0 4px 12px rgba(124,58,237,0.35)",fontSize:12}}>⚡</div>
            <span style={{fontWeight:800,fontSize:15,letterSpacing:"-0.02em",background:"linear-gradient(135deg,#7c3aed,#2563eb)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>APEX</span>
          </div>
        </div>
        <div style={{display:"flex",overflowX:"auto",gap:2,padding:"6px 8px 8px",scrollbarWidth:"none",WebkitOverflowScrolling:"touch"}}>
          {navItems.map(n=>(<button key={n.id} onClick={()=>setView(n.id)} className="nav-btn" style={{display:"flex",flexDirection:"column",alignItems:"center",gap:2,padding:"7px 14px",borderRadius:12,border:"none",cursor:"pointer",background:view===n.id?"rgba(124,58,237,0.12)":"transparent",color:view===n.id?"#7c3aed":"#94a3b8",fontWeight:view===n.id?700:500,transition:"all 0.2s",fontFamily:"inherit",flexShrink:0,minWidth:52}}><span style={{fontSize:16}}>{n.icon}</span><span style={{fontSize:10,letterSpacing:"0.04em",fontWeight:600}}>{n.label}</span></button>))}
        </div>
      </nav>
...
const pg={maxWidth:600,margin:"0 auto",padding:"20px 16px"};
...
const CSS=`
  ...
  *{box-sizing:border-box;}body{margin:0;}button,input,textarea{font-family:inherit;}
  ...
  .glass-card{transition:box-shadow 0.2s;}.nav-btn:hover{background:rgba(124,58,237,0.08)!important;}
